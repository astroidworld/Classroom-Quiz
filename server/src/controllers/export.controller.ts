...
export const exportSessionCSV = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const csvData = session.answers.map((ans) => ({
      ...
      'Selected Option': ans.selectedOption.text,
...
export const exportSessionPDF = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const addDetailLine = (label: string, value: string) => {
      ...
      doc.font('Helvetica').text(value, 150, detailsRow);
      detailsRow += 20;
    };
    ...
    session.quiz.questions.forEach((q) => {
      ...
      doc.roundedRect(45, questionY, 515, 110, 6).fillColor('#fafafa').fill();
      doc.roundedRect(45, questionY, 515, 110, 6).strokeColor(borderGrey).lineWidth(0.5).stroke();
      ...
      // Question Text
      doc.fillColor(textColor);
      doc.fontSize(10).font('Helvetica-Bold').text(q.text, 55, questionY + 25, { width: 500, height: 28 });
      ...
      // Stats block
      doc.fontSize(8).font('Helvetica-Bold').fillColor('#64748b').text('ACCURACY', 55, questionY + 60);
      doc.fontSize(14).font('Helvetica-Bold').fillColor(accuracy >= 80 ? correctGreen : accuracy >= 50 ? '#d97706' : incorrectRed).text(`${accuracy}%`, 55, questionY + 70);

      doc.fontSize(8).font('Helvetica-Bold').fillColor('#64748b').text('AVG RESPONSE TIME', 160, questionY + 60);
      doc.fontSize(14).font('Helvetica-Bold').fillColor(textColor).text(`${avgSpeed}s`, 160, questionY + 70);

      // Options breakdown
      let optionX = 300;
      doc.fontSize(8).font('Helvetica-Bold').fillColor('#64748b').text('OPTION DISTRIBUTION', optionX, questionY + 60);
      
      let optTextRows = questionY + 72;
      ...
      // Explanation
      if (q.explanation) {
        doc.fontSize(7).font('Helvetica-Oblique').fillColor('#64748b').text(`Explanation: ${q.explanation.substring(0, 110)}${q.explanation.length > 110 ? '...' : ''}`, 55, questionY + 95, { width: 495 });
      }
    ...
    session.participants.forEach((p) => {
      ...
      sortedAnswers.forEach((ans) => {
        doc.rect(45, studentTableY, 515, 20).fillColor(lightGrey).fill();
        doc.rect(45, studentTableY, 515, 20).strokeColor('#e2e8f0').lineWidth(0.5).stroke();

        doc.fontSize(8).font('Helvetica-Bold').fillColor(textColor).text(`Q${ans.question.order}`, 55, studentTableY + 6);
        doc.font('Helvetica').text(ans.question.text.substring(0, 36) + (ans.question.text.length > 36 ? '...' : ''), 85, studentTableY + 6);
        doc.text(ans.selectedOption.text.substring(0, 28) + (ans.selectedOption.text.length > 28 ? '...' : ''), 280, studentTableY + 6);
        
        doc.font('Helvetica-Bold').fillColor(ans.isCorrect ? correctGreen : incorrectRed).text(ans.isCorrect ? 'Correct' : 'Incorrect', 430, studentTableY + 6);
        doc.font('Helvetica').fillColor(textColor).text(`${Math.round(ans.responseTimeMs / 100) / 10}s`, 485, studentTableY + 6);
        doc.font('Helvetica-Bold').fillColor(secondaryColor).text(`${ans.pointsAwarded}`, 525, studentTableY + 6);
      ...
      if (sortedSlowest.length > 0) {
        ...
        sortedSlowest.forEach((ans, idx) => {
          doc.fontSize(8).font('Helvetica-Bold').fillColor(textColor).text(`${idx + 1}. Q${ans.question.order}: "${ans.question.text}"`, 45, studentTableY);
          doc.font('Helvetica').fillColor('#64748b').text(`   Response Time: ${Math.round(ans.responseTimeMs / 100) / 10}s   |   Status: ${ans.isCorrect ? 'Correct' : 'Incorrect'}`, 45, studentTableY + 11);
          studentTableY += 25;
        });
      }
    });

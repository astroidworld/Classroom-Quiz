...
export default function SessionDashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white pb-16">
      {/* Header */}
      <header className="glass-panel sticky top-0 z-40 px-6 py-4 mb-8 border-b border-white/5 bg-slate-950/80 backdrop-blur-md shadow-xl">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link 
              to="/host/dashboard"
              className="group text-slate-400 hover:text-white p-2 rounded-xl bg-slate-900/60 hover:bg-slate-900 border border-slate-800/80 hover:border-slate-700 transition-all active:scale-95"
            >
              <ArrowLeft className="w-4 h-4 transition-transform group-hover:-translate-x-0.5" />
            </Link>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-indigo-400 font-extrabold uppercase tracking-widest">Session Reports</span>
                {data.mode && (
              ...
              <h1 className="text-xl font-bold font-display tracking-tight mt-0.5 bg-clip-text text-transparent bg-gradient-to-r from-white via-indigo-100 to-slate-400 text-glow">{data.quizTitle}</h1>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            {sessionId && (
              <div className="flex gap-2.5">
                <button
                  ...
                  className="bg-slate-900/80 hover:bg-slate-800 border border-slate-800 hover:border-slate-700 text-slate-300 hover:text-white font-bold py-2 px-4 rounded-xl transition-all text-xs flex items-center gap-1.5 shadow-md active:scale-95 disabled:opacity-50"
                ...
                <button
                  ...
                  className="bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white border border-indigo-400/20 hover:border-indigo-400/40 font-bold py-2 px-4 rounded-xl transition-all text-xs flex items-center gap-1.5 shadow-lg shadow-indigo-500/10 hover:shadow-indigo-500/20 active:scale-95 disabled:opacity-50"
            ...
            {data.joinCode && (
              <div className="bg-slate-950/80 border border-indigo-500/25 px-4 py-2 rounded-xl text-xs font-bold text-indigo-300 shadow-[inset_0_1px_3px_rgba(0,0,0,0.8)]">
      ...
      <nav className="max-w-7xl mx-auto px-6 mb-8">
        <div className="flex justify-start md:justify-center bg-slate-950/60 backdrop-blur-md p-1.5 rounded-2xl border border-white/5 w-full max-w-full overflow-x-auto gap-1.5 scrollbar-none shadow-2xl shadow-black/80">
          {[
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all duration-300 ease-out shrink-0 select-none border active:scale-[0.97] hover:-translate-y-0.5 ${
                  isActive
                    ? 'bg-gradient-to-r from-indigo-500 via-indigo-600 to-purple-600 text-white shadow-[0_0_20px_rgba(99,102,241,0.4)] border-indigo-400/20'
                    : 'text-slate-400 hover:text-white bg-transparent border-transparent hover:bg-slate-900/60'
                }`}
              >
                <span className={`transition-transform duration-300 ${isActive ? 'scale-110 text-indigo-200' : 'opacity-70 group-hover:scale-110'}`}>
                  {t.icon}
                </span>
                {t.label}
              </button>
                              ...
                              <tr key={ans.questionId} className="hover:bg-slate-900/20">
                                ...
                                <td className="py-4">
                                  <div className="flex flex-col">
                                    <span className="font-black text-indigo-400">{ans.pointsAwarded} pts</span>
                                    {ans.earlyBonus > 0 && (
                                      <span className="text-[10px] font-bold text-emerald-400">+{ans.earlyBonus} early bonus</span>
                                    )}
                                    {ans.penalty > 0 && (
                                      <span className="text-[10px] font-bold text-red-400">-{ans.penalty} penalty</span>
                                    )}
                                  </div>
                                </td>
                              </tr>
                        ...
                        <tr key={row.participantId} className="hover:bg-slate-900/30">
                          ...
                          {row.answers.map((cell) => {
                            ...
                            if (cell.isCorrect === true) {
                              ...
                              titleText = `Correct! Choice: "${cell.selectedOptionText}" (${cell.responseTimeSec}s) | Score: +${cell.pointsAwarded || 0} pts${cell.earlyBonus && cell.earlyBonus > 0 ? ` (+${cell.earlyBonus} early bonus)` : ''}`;
                            } else if (cell.isCorrect === false) {
                              cellBg = 'bg-red-500/15 border-red-500/20 text-red-400 hover:bg-red-500/35';
                              titleText = `Incorrect! Choice: "${cell.selectedOptionText}" (${cell.responseTimeSec}s) | Penalty: -${cell.penalty || 0} pts`;
                            }
                            return (
